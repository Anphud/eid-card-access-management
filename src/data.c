		...
		if (pC_GetFunctionList != NULL)
		{
			...
			if (retVal == CKR_OK)
			{
				...
				if (retVal == CKR_OK)
				{
					...
					if ((retVal == CKR_OK) && (slot_count > 0) && !gIsCardInserted)
					{
						...
						if (slotIds != NULL)
						{
							...
							if (retVal == CKR_OK)
							{
								...
								for (slotIdx = 0; slotIdx < slot_count; slotIdx++)
								{
									...
									if (retVal == CKR_OK)
									{
										loadCard(pFunctions, session_handle);
										gIsCardInserted = 1;

										//close the session
										if (retVal == CKR_OK) retVal = (pFunctions->C_CloseSession) (session_handle);
										else (pFunctions->C_CloseSession) (session_handle);
									}
								}//end of for loop
							}
							free(slotIds);
